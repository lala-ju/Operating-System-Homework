...
walkaddr(pagetable_t pagetable, uint64 va)
{
  ...
  if(*pte & PTE_S)
  {
    ...
    copyout(myproc()->pagetable, va, pa, PGSIZE);
...
void print(pagetable_t page, int level, bool end0, bool end1, uint64 ori)
{
  if(level == 0)
    printf("page table %p\n", page);
  int end = 0;
  ...
  for(int i = 0; i < 512; i++)
  {
    ...
    if(pte & PTE_V || pte & PTE_S)
    {
      ...
      printf("+-- %d: pte=%p va=%p ", i, page+i, va);
      ...
      if(pte & PTE_V)
      {
        ...
        {
          if(level == 0)
            print((pagetable_t) pa, level+1, end <= i, true, va);
          if(level == 1)
            print((pagetable_t) pa, level+1, end0, end <= i, va);
        } 
      }
    }
  }
...
int madvise(uint64 base, uint64 len, int advice) {
  ...
  switch(advice)
  {
    ...
    case(MADV_WILLNEED):
    {
      ...
      uint64 bot = PGROUNDDOWN(base);
      uint64 up = PGROUNDDOWN(base+len-1);
      for(uint64 i = bot; i <= up; i+= PGSIZE)
      {
        ...
        else if(!(*pte & PTE_V))
        {
          ...
          if (mem == 0) {
            ...
            memset(mem, 0, PGSIZE);
            if(mappages(myproc()->pagetable, i, PGSIZE, (uint64)mem, PTE_V|PTE_U|PTE_R|PTE_W|PTE_X) != 0) {
              *pte |= PTE_V;
              kfree(mem);
              return -1;
            }
