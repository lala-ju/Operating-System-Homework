...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NINDIRECT){
    ...
    if((addr = ip->addrs[NDIRECT + (bn/NUMM)]) == 0)
      ip->addrs[NDIRECT + (bn/NUMM)] = addr = balloc(ip->dev);
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
    if((addr = a[(bn%NUMM)]) == 0){
      a[(bn%NUMM)] = addr = balloc(ip->dev);
      log_write(bp);
    }
    brelse(bp);
    return addr;
...
itrunc(struct inode *ip)
{
  ...
  if(ip->addrs[12])
  {
    ...
    for(j = 0; j < NUMM; j++)
    {
      if(a[j])//second layer have been allocated
      {
        struct buf *bpp;
        uint *aa;
  ...
  while((path = skipelem(path, name)) != 0){
    ...
    if(ip->type == T_SYMLINK)
    {
      ...
      while(ip->type == T_SYMLINK && count < 20)
      {
        ...
        readi(ip, 0, (uint64)p, sizeof(int), len+1);
        iunlockput(ip);
        if((ip = namei(p)) == 0)
        {
          //end_op();
